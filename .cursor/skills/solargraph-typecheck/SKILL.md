...
description: >-
  ...
  after a Solargraph upgrade in checkoff.
version: 1.0.0
---
...
- Long raw Solargraph messages: start on the `# @sg-ignore` line, continue on the next comment line (indented), e.g. `# @sg-ignore Foo#bar` then `#   return type could not be inferred`.
