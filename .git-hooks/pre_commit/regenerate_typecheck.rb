# typed: true
# frozen_string_literal: true

require 'overcommit'
require 'overcommit/hook/pre_commit/base'

module Overcommit
  module Hook
    module PreCommit
      # Regenerates Sorbet gem RBIs and related lockfiles when Gemfile.lock is committed.
      class RegenerateTypecheck < Base
        TYPECHECK_PATHS = [
          'sorbet/rbi/gems',
          'sorbet/rbi/annotations',
          'sorbet/rbi/todo.rbi',
          'rbs_collection.lock.yaml',
        ].freeze

        # @return [Symbol, Array<Symbol, String>]
        def run
          result = execute(%w[make update-typecheck-from-lock])
          unless result.success?
            return [:fail, result.stdout + result.stderr]
          end

          stage_result = execute(['git', 'add', '-A', '--', *TYPECHECK_PATHS])
          return :pass if stage_result.success?

          [:fail, stage_result.stdout + stage_result.stderr]
        end
      end
    end
  end
end
